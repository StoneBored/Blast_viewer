    ...
    def test_1(self):
        ...
        # EQUATION
            ...
            for px, py, charge in p_charge:
                ...
                z = 10 * np.log10((((( r / (charge**(1 / 3)) )**self.asite) * self.kasite)/(0.00002))**2)
                z_log += 10**(z/10)
            z_total = 10*np.log10(z_log)

            Z= np.array(z_total).reshape(len(y),len(x))
            z1 = [10 * np.log10(((((i/(10**(1/3)))**self.asite) * self.kasite)/(0.00002))**2) for i in np.arange(0.1, (N+1)*np.sqrt(2))]
            ...
            for px, py, charge in p_charge:
                r = np.sqrt((((px-X)**2)+((py-Y)**2)))
                z = 10 * np.log10(((((( r / (charge**(1 / 3)) )**-1.45) * 516)/(0.00002))**2))
                z_log += 10**(z/10)
                z_suma = 10*np.log10(z_log)
            z_total = 0.00002 * 10**(z_suma/20)
    ...
    # get closest point with known data
        ...
        for value in bars:
            ...
            plt.text(value.get_x() + value.get_width()/2.,
                    1.002*height,'%s' % float(height), ha='center', va='bottom')
        
        if (self.var1 == "dBL"):
            plt.ylim(z_total.min()/1.1 , max(z_values)*1.25)
            plt.ylabel('Pressure Level (dBL)')
            x1 = [0, 1, 2, 3, 4 ]
            curve_1 = [133, 133, 133, 133, 133]
            curve_2 = [125, 125, 125, 125, 125]
            curve_3 = [120, 120, 120, 120, 120]
            plt.plot(x1, curve_1,'b--',label="133 dBL: Maximum for Structural Damage Level" )
            plt.plot(x1, curve_2,'r--',label="125 dBL: Maximum for Human Comfort Level" )
            plt.plot(x1, curve_3,'g--',label="120 dBL: Annoying")
            fontP = FontProperties()
            fontP.set_size('x-small')
            plt.legend(loc='upper right', prop=fontP)
            ...
            plt.yscale('log')
            plt.ylabel('Pressure (Pa)',labelpad=0)
            plt.ylim(z_total.min()/10 , max(z_values)*100)
            x1 = [0, 1, 2, 3, 4, ]
            curve_1 = [89, 89, 89, 89, 89]
            curve_2 = [35, 35, 35, 35, 35]
            curve_3 = [20, 20, 20, 20, 20]
            plt.plot(x1, curve_1,'b--',label="89 Pa: Maximum for Structural Damage Level" )
            plt.plot(x1, curve_2,'r--',label="35 Pa: Maximum for Human Comfort Level" )
            plt.plot(x1, curve_3,'g--',label="20 Pa: Annoying")
            fontP = FontProperties()
            fontP.set_size('x-small')
            plt.legend(loc='upper right', prop=fontP)
